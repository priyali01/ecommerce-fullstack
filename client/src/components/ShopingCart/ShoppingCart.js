...
import React, { useEffect, useState } from "react";
import "./ShoppingCart.scss"; // keep your styling

const ShoppingCart = (props) => {
  // We’ll still show passed cartProducts if available
  const [cartProducts, setCartProducts] = useState(props.cartProducts || []);
  
  // Disable backend calls for now
  useEffect(() => {
    // If props.cartProducts changes, update local state
